...
- **One layer stack.** Glass panels and collage images share a single order, so an image
  can sit above a panel or below it.
...
| Duplicate panel | ⌘D |
| Delete panel | ⌘⌫ |
| Change stacking order | ⌘] forward, ⌘[ backward, ⇧⌘] to front, ⇧⌘[ to back |
...
Panels and collage images live in one stack, drawn bottom to top. Compositing order, per
...
Each panel samples its blur from a **backdrop**: the base image plus the collage images
that sit *below that panel*, and no glass at all. Both halves of that rule matter. Leaving
glass out means overlapping panels never double-blur each other. Taking only the images
below means an image stacked on top of a panel doesn't bleed into it — otherwise the glass
would show a ghost of something it cannot actually cover. The renderer keeps a second
context holding just the base and the images, and snapshots it whenever it reaches a panel.

`AppState.layerOrder` is the single source of truth for stacking; the `panels` and `photos`
arrays are only containers, and their own order means nothing. Keeping them separate lets
the Inspector bind straight to `$state.panels[i]` instead of reaching through an
...
Sources/Glassmorphism/
  ...
  Tools/ci-run.sh       surfaces CI failures as annotations (logs need auth, these don't)
Resources/
