...
- **单一图层堆叠**：毛玻璃与拼贴图片共用同一份顺序，图片可以叠在玻璃之上，也可以在之下。
...
面板与拼贴图片在同一个堆叠里，由下而上画。每块面板的合成顺序：外侧投影 → 圆角路径 clip →
贴**整张画布的模糊版**（铺满整幅，所以玻璃底下的内容与背后的画面精确对齐）→ 叠色 →
噪点（overlay）→ 描边 → CoreText 画文字。

每块面板的模糊都采样自 **backdrop**：底图，加上**排在这块面板底下**的拼贴图片，不含任何玻璃。
两个条件都有意义。不含玻璃，面板重叠时才不会重复模糊；只取底下的图片，叠在面板之上的图片
才不会渗进去 —— 否则玻璃上会出现一份它其实盖不到的残影。渲染器为此另外维护一个只有底图与
图片的 context，走到面板时才快照。

`AppState.layerOrder` 是叠放顺序的唯一依据；`panels` 与 `photos` 只是容器，它们自身的
顺序不代表任何意义。分开存是为了让 Inspector 还能直接绑 `$state.panels[i]`，
不必为了排序去穿透 enum 的关联值。
...
Sources/Glassmorphism/
  ...
  Tools/ci-run.sh       把 CI 失败信息送成 annotation（日志要认证，annotation 不用）
Resources/
